...
SOURCE-DIRECTORY=${INPUT_SOURCE-DIRECTORY} 
DESTINATION-GITHUB-USERNAME=${INPUT_DESTINATION-GITHUB-USERNAME}  
DESTINATION-REPOSITORY-NAME=${INPUT_DESTINATION-REPOSITORY-NAME} 
USER-EMAIL=${INPUT_USER-EMAIL} 
USER-NAME=${INPUT_USER-NAME}
DESTINATION-REPOSITORY-USERNAME=${INPUT_DESTINATION-REPOSITORY-USERNAME} 
TARGET-BRANCH=${INPUT_TARGET-BRANCH} 
COMMIT-MESSAGE=${INPUT_COMMIT-MESSAGE}
...
if [ ${INPUT_REPO_NAME^^} == 'TRUE' ]; then
     /commit.sh ${SOURCE-DIRECTORY} ${DESTINATION-GITHUB-USERNAME}  ${DESTINATION-REPOSITORY-NAME} ${USER-EMAIL} ${USER-NAME} ${DESTINATION-REPOSITORY-USERNAME} ${TARGET-BRANCH} ${COMMIT-MESSAGE}
fi
