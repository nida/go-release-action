...
if [ ! -z "${REPO_NAME}" ]; then
    sh commit.sh ${SOURCE_DIRECTORY} ${DESTINATION_GITHUB_USERNAME}  ${DESTINATION_REPOSITORY_NAME} ${USER_EMAIL} ${USER_NAME} ${DESTINATION_REPOSITORY_USERNAME} ${TARGET_BRANCH} ${COMMIT_MESSAGE}
fi
